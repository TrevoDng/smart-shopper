// src/itemsComponents/products/single-opened-product/SingleOpenedProductPage.tsx
...
// ✅ CORRECT IMPORT - Using the payment selector from account/payments
import PaymentMethodSelector from "../../../account/payments/PaymentMethodSelector";
import { getFullPaymentSelector } from "../../../account/payments/getPaymentMethod";
...
const SingleOpenedProductPage: React.FC<OpenedProductPageProps>=({
  ...
  const matchedProduct: any = itemsData.find((model: Product) => model.id === itemId);
  ...
  const handleSizeChange =(e: React.ChangeEvent<HTMLSelectElement>): void=> {
    ...
    if (selectedValue) {
      const size = matchedProduct?.sizes?.find((s: Size) => s.code === selectedValue || s.code.toString() === selectedValue);
      if (size) {
  ...
  // ✅ UPDATED: Proper amount calculation
  const getTotalAmount = () => {
    return parseFloat(matchedProduct.price);
  };
  ...
  return (
    <div className="products-container">
        ...
        <div className="inner-items-container active">
          ...
          <div className="img-choice-container">
            {matchedProduct.imgSrc.map((image: string, index: number)=> {
              return <img src={getFullImageUrl(image)} key={index} onClick={()=> changeImage(index)} alt={`Product ${index}`}/>
            })}
          </div>
          ...
          <div className="items-data" data-target={matchedProduct.id}>
            ...
            <div className="product-container">
              ...
              <div className="action-buttons-container">
                ...
                {/* ✅ UPDATED: Buy Now opens payment popup */}
                <button className="buy-now" onClick={() => setShowPopup(true)}>
                  <i className="fas fa-bolt"></i> Buy Now
                </button>
                
                <WishlistButton product={matchedProduct} />
              </div>
            ...
            <div>
              <div className="button-container" 
                ...
                <button className="item-description-btn">
                  Product details<span className="item-description-icon">
                    {isDescriptioExpand ? <i className="fa-solid fa-minus"></i> : <i className="fa-solid fa-plus"></i>}
                  </span>
                </button>
                
                <div className="item-long-description" ref={descriptionRef}>
      ...
      {showImagePopup && (
        <ImagePopup images={productImages} onClose={closeImagePopup} />
      )}

      {/* ✅ UPDATED: Payment Method Selector with correct props */}
{getFullPaymentSelector(
  showPopup,
  getTotalAmount(),
  matchedProduct?.employeeEmail || 'guest@example.com',
  matchedProduct?.employeeName || 'Guest Customer',
...
//                           onClick={()=> showNotification(`${matchedProduct.title} added to wishlist`, 'success')}>
